...
class Wheel(gui.Widget):
    def __init__(self, parent, actors, **args):
        ...
        self.wheel = tichy.Image(WheelDesign.path('wheel.png'), Vect(480, 288)).view(box)
        self.actors = []
    ...
    def __set_angle(self, value):
        ...
        for i, actor in enumerate(self.actors):
            angle = value + i * pi / 5
            actor.pos = Vect(int(r * cos(angle)), -int(r * sin(angle))) + self.center
    ...
    def motion_task(self, pos):
        while True:
            ...
            if e == 0:
                break
            pos = args[0]
            # we need to compute the angle between the original click pos and the new pos
            # But in fact I am too lazy so I will use an approximation
            # TODO: make it faster by using a step value 
            self.angle = (self.click_angle + (pos - self.click_pos).x / float(self.radius))
