...
class FreeSmartPhonePrefs(PrefsService):
    ...
    class Service(object):
        def __init__(self, prefs, name):
            ...
            self.service = prefs.bus.get_object('org.freesmartphone.opreferencesd', path)
    ...
    def __init__(self):
        ...
        try:
            ...
            self.prefs = self.bus.get_object('org.freesmartphone.opreferencesd', '/org/freesmartphone/Preferences')
        except Exception, e:
        ...
        def __getitem__(self, name):
            for profile in self.prefs.activated_profiles:
                try:
                    ...
                    logger.info("Get %s.%s using profile %s", self.name, name, profile)
                    return value
                except KeyError:
                    pass
            raise KeyError, name
