...
class TypeFilter(Filter):
    ...
    def __cmp__(self, v):
        """We define the cmp method so that the more general rules are 'lower' than more 'specifics'"""
...
class Style(dict, Item):
    """ A Style is a dictionary of key -> values with parenting property
        ...
        When you apply a style to a widget, you get the actual style dictionary
        that can be used by any painter to draw the wiget.
        Here are the rules :
    ...
    @classmethod    # TODO: this is not really good, we shouldn't have to call this 
    def create(cls):
    ...
    def apply(self, widget, up = True): # XXX: also apply to Style attribute (children-style, etc..)
        ret = {}
    ...
    def from_dict(cls, parent, d):
        ...
        # sort the parts do that the more general are at the beginning at will be overriden by the more specific ones
