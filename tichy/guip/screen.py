...
class Screen(Window):
    """Main widget for everything
       ...
       Screen is the widget that is at the end of the chain when we emit "need-redraw".
       It is the only widget that start a redrawing sequence.
       It may seem not optimized to always redraw from the top widget, but the idea is that you can then
       have transparency in any widgets 
    """
    def __init__(self, events_source, painter, **kargs):
        self.redraw_rect = None
        super(Screen, self).__init__(None, events_source = events_source, modal = False, **kargs)
    ...
    def draw(self):
        ...
        self.painter.set_mask(self.redraw_rect) # TODO: use virtual attribute
    ...
    def need_redraw(self, rect):
        self.redraw_rect = rect if not self.redraw_rect else self.redraw_rect.merge(rect)
