...
class Box(Frame):
    ...
    def __get_spacing(self):
        return self.__spacing if self.__spacing is not None else \
            self.style_dict.get('spacing', 0)
    ...
    def resize(self):
        ...
        if self.children:
            min_size = min_size.set(
                axis, sum(c.min_size[axis] + self.spacing \
    ...
    def organize(self):
        ...
        while True:
            children_need = [c for c in self.children
                             if c.optimal_size[axis] > c.size[axis]]
