...
from ..events_loop import MainLoop
...
class SdlPainter(Painter):
    """ Painter Engine that use SDL for rendering
        ...
        SDL can be used with X11, or direct frame buffer.
        It only offers simple blitting functions, but do it very efficiently.
    """
    ...
    def to_surface(self, surface):
        """Return a engine similar to this one but drawing into a given surface"""
    ...
    def fill(self, color, size = None):
        if size:
            self.surface.fill(color, (self.pos.x, self.pos.y, size[0], size[1]))
        else:
